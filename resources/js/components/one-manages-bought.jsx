...
export default forwardRef(function TextInput({ type = 'text', transcation, className = '', isFocused = false, ...props }, ref) {
    ...
    const [estimatedDelievry, setEstimatedDelievry] = useState(new Date('2025-06-24'));
    ...
    async function deleteInfo(id) {
        const res = router.delete(`/managesoldinfo/${id}`);
    }
    ...
    return (
        ...
        >
            ...
            {user !== null && <div className="mb-1 text-gray-700 dark:text-gray-300">Send the item to service point: {user.closestService}</div>}
            {estimatedDelievry && (
                <div className="mb-1 text-gray-700 dark:text-gray-300">Estimated delievery at: {estimatedDelievry.toLocaleDateString()}</div>
            )}
